...
use checker::election_actor::{NolerActor, NolerMsg, NolerMsg::*};
...
impl Actor for NolerOpActor {
    type Msg = NolerMsg<Ballot, NolerValue, ConfigSr, checker::noler_msg_checker::NolerMsg>;
    ...
    fn on_msg(
        ...
        match msg {
            ...
            ResponseVote(ballot) if ballot == (state.ballot.0 + 1, state.ballot.1 + 1) => {
                let state = state.to_mut();
            ...
            Config(ballot, config) if state.role != Role::Leader => {
                let state = state.to_mut();
            ...
            RequestConfig(ballot) if state.status == NORMAL && state.role == Role::Leader => {
                let state = state.to_mut();
                ...
                if ballot > state.ballot {
                    ...
                    o.send(src, Config(ballot, state.config.clone()));
                }
            }

            HeartBeat(ballot, profile) if state.status == NORMAL && (state.role == Role::Candidate || state.role == Role::Witness) => {
                let state = state.to_mut();
                ...
                if ballot >= state.ballot {
                        ...
                        Role::Candidate => {
                            ...
                            if (p2 - p1) > 51 {
                                ...
                                for &dst in &self.peer_ids {
                                    ...
                                    o.send(dst,
                                        RequestVote((state.ballot.0 + 1, state.ballot.1 + 1), 
                    ...
                    o.send(state.leader.unwrap().0, RequestConfig(ballot));

                    return;
                }
            }

            PollLeader(ballot) if state.status == NORMAL && state.role == Role::Leader => {
                let state = state.to_mut();

                if ballot <= state.ballot {
                    o.send(src, PollLeaderOk(ballot, state.monitor.get(&src).unwrap().get_x()));
                }
            ...
            PollLeaderOk(ballot, profile) if state.status == NORMAL && state.role == Role::Candidate => {
                let state = state.to_mut();
               ...
               LeaderInitTimeout => if state.role == Role::Member  {
                ...
                LeaderLeaseTimeout => if state.role == Role::Leader {
                    ...
                    for &dst in &self.peer_ids {
                       ...
                       o.send(dst,
                        HeartBeat(state.ballot, state.monitor.get(&dst).unwrap().get_x())
                        );
                    }
                    o.set_timer(LeaderLeaseTimeout, 
                        Duration::from_secs(LEADER_LEASE_TIMEOUT)..Duration::from_secs(LEADER_LEASE_TIMEOUT));
...
#[derive(Clone, Debug)]
struct NolerOpModelCfg {
    //client_count: usize,
    peer_count: usize,
    config: ConfigSr,
    network: Network<<NolerOpActor as Actor>::Msg>,
}

impl NolerOpModelCfg {
    fn new(peer_count: usize, start_port: u16, network: Network<<NolerOpActor as Actor>::Msg>) -> Self {
        let mut replicas = Vec::new();
    ...
    fn into_model(
        ...
        NolerActor<NolerOpActor>,
        Self,
        LeaderElectionTester<Id, Noler<Ballot, ConfigSr>>>

    {
        ActorModel::new(
            self.clone(),
            LeaderElectionTester::new(Noler((0, 0), None)),
        )
        .actors((0..self.peer_count).map(|i| {
            NolerActor::Server(NolerOpActor {
                peer_ids: model_peers(i, self.peer_count),
            })
        }))
        .init_network(self.network)
        .property(Expectation::Sometimes, "A leader will be elected", |_, state| {
            state.network.iter_deliverable().any(|env| {
                if let Config(ballot, config) = env.msg {
        ...
        //     false
        // })
        .record_msg_in(NolerMsg::record_returns)
        .record_msg_out(NolerMsg::record_invocations)
    }
}
...
mod test {
    ...
    fn can_model_noler() {
        ...
        let checker = NolerOpModelCfg::new(
            peer_count, start_port, Network::new_unordered_nonduplicating([])) //new_unordered_nonduplicating
            .into_model()
            .checker()
            .spawn_dfs()
            .join();
...
fn main() -> Result<(), pico_args::Error> {
    ...
    match args.subcommand()?.as_deref() {
        Some("check") => {
            ...
            let check = NolerOpModelCfg::new(peer_count, start_port, network);

            // let actor_model = NolerOpModelCfg::new(
            //     peer_count, start_port, Network::new_unordered_nonduplicating([])) //new_unordered_nonduplicating
            //     .into_model();
    
            // let init_states = actor_model
            //     .init_states()
            //     .iter()
            //     .map(|s| serde_json::to_string_pretty(&s)).collect::<Vec<_>>();
    
            // log::info!("Initial states: {:#?}", init_states);
        ...
        Some("explore") => {
            ...
            let explore = NolerOpModelCfg::new(peer_count, start_port, network);
        ...
        _ => {
            ...
            println!(
                "NETWORK: {}",
                Network::<<NolerOpActor as Actor>::Msg>::names().join(" | ")
            );
        }
    }
